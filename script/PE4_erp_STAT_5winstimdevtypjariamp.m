...
win=[];
win.mmn1=minTime-0.05; win.mmn2=minTime+0.05;
...
winlenght=0.035;%[0.025,0.03,0.035,0.04,0.05];
clc
win=[];
diary('output.txt');
for iwin=1:length(winlenght)
Diff_ga=[];
for idev=1:2   
    for md=1:4
...
%}
    ...
    idxVar = 1;
        ...
        for day = 1:4
            varNames{idxVar,1} = sprintf('Day%d', day);
            idxVar = idxVar + 1;
        end
   

    %% Convert data_2D to a table and set variable names
    T = array2table(data_2D(:,5:8), 'VariableNames', varNames);
...
rm = fitrm(T, sprintf('%s-%s ~ 1', varNames{1}, varNames{4}), 'WithinDesign', WithinDesign);

    % Repeated measures ANOVA (including factors: Stimulus, Day, and their interaction)
    ranovatbl = ranova(rm, 'WithinModel', 'Day');
    % 2）计算 Day 的 η?
SS_effect_Day = ranovatbl.SumSq(3);
SS_error_Day   = ranovatbl.SumSq(4);
itest=itest+1;
  Fstat{itest}.etaSq_Day(1) = SS_effect_Day / (SS_effect_Day + SS_error_Day);
 Fstat{itest}.F=ranovatbl.F(3);
  Fstat{itest}.pValue=ranovatbl.pValue(3);
      etaSq_DayAll(itest,1) = Fstat{itest}.etaSq_Day(1);
    FAll(itest,1)        = Fstat{itest}.F;
    pAll(itest,1)        = Fstat{itest}.pValue;
...
comp{itest}.cohenD =( compDay.Difference./ compDay.StdErr)./sqrt(14);         % 配对 Cohen's d
...
clear ResultsTable 
ResultsTable = table(FAll, pAll,etaSq_DayAll, ...
    'VariableNames', {'F','pValue','etaSq_Day'});
end
disp(ResultsTable)
diary off;
