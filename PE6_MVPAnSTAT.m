...
filename={[filler1,filler2,filler3,'day1_nsubDEVnSTDmvpa_re2.mat'],[filler1,filler2,filler3,'day2_nsubDEVnSTDmvpa_re2.mat'],...
    [[filler1,filler2,filler3],'day3_nsubDEVnSTDmvpa_re2.mat'],[filler1,filler2,filler3,'day4_nsubDEVnSTDmvpa_re2.mat']};
eeglab
...
for idev=1:2
    ...
    for i1stday=1%:3
        for i2ndday=i1stday+1:4
            data=[];mean_result=[];
            load([method,'dev_',num2str(idev),num2str(i1stday),num2str(i2ndday),'_eachtwoday_MVPA_result_meeg_dev_re3.mat']);
            results=result_meeg;
            for isub=1:14
...
save([filler1,filler2,filler3,filler4,method,'_alltime_mvpa_permutation_diff_ttest.mat'],'pMMN','pP3a');
%% waveform
...
for idev=1:2
    ...
    for i1stday=1%:3
        for i2ndday=i1stday+1:4
            ...
            X=[];Y=[];upper=[];lower=[];
            y=mean_auc;
            err=(mean_result.perf_std{1, 1})/sqrt(14);
            ...
            idx = (x >= 0.188) & (x <= 0.248);
            plot(time(idx), mean_auc(idx), 'LineWidth', 1, 'Color', 'k');hold on;
            idx = (x >= 0.288) & (x <= 0.348);
            plot(time(idx), mean_auc(idx), 'LineWidth', 1, 'Color', [202,0,32]./255);
            %}
            %%
            if pMMN(i,idev) < 0.05/3
                text(0.208, 0.521, '*', 'Color', 'k', 'FontSize', 12, ...
                    'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
            end
            if pP3a(i,idev) < 0.05/3
                text(0.328, 0.521, '*', 'Color', [202,0,32]./255, 'FontSize', 12, ...
                    'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
            end            
            %%
...
%                 'FontSize', 10,'fontweight', 'bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
    ...
    filename = sprintf('MVPAdiff%d.tiff', idev);
